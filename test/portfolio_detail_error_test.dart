...
void main() {
  testWidgets('PortfolioDetailScreen shows error state on failure',
      (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: ChangeNotifierProvider<ContentProvider>(
          create: (_) => _ThrowingContentProvider(),
          child: const PortfolioDetailScreen(slug: 'project-komunitas'),
        ),
      ),
    );
    ...
    expect(find.text('Gagal memuat detail portfolio.'), findsOneWidget);
...
class _ThrowingContentProvider extends ContentProvider {
  ...
  Future<PortfolioItem?> getPortfolioDetail(String slug) async {
    throw Exception('Gagal memuat detail portfolio.');
  }
}
